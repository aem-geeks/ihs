...
export async function loadOverrideBlock(block) {
...
export async function loadOverrideBlocks(main) {
  updateSectionsStatus(main);
  const blocks = [...main.querySelectorAll('div.block.override')];

  for (let i = 0; i < blocks.length; i += 1) {
    // eslint-disable-next-line no-await-in-loop
    console.log(blocks[i]);
    await loadOverrideBlock(blocks[i]);
    updateSectionsStatus(main);
  }
}
